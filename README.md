...
V0.4.4에서는 Web 저장에 순차 revision을 추가하고 파일·브라우저 현재본·직전
백업·조각 쿠키 복구본 중 가장 최신 정상본을 선택합니다. 수동 저장 결과에도
실제로 성공한 브라우저 저장 경로가 표시됩니다. 저장 데이터는 브라우저 보안상
접속 주소별로 분리되므로 같은 주소로 다시 접속해야 이어집니다.
